...
4. Check the `read:packages` + `write:packages` scopes (should auto-enable `repo` scopes)
...
```bash
npm login --scope=@kernl-sdk --registry=https://npm.pkg.github.com
```
...
- **`@kernl-sdk/core`**: Main framework for building AI agents
- **`@kernl-sdk/ai`**: Vercel AI SDK adapters (supports Anthropic, OpenAI, Google)
- **`@kernl-sdk/protocol`**: Core protocol definitions
- **`@kernl-sdk/cli`**: CLI for scaffolding projects
...
3. Try logging out and back in:
   ```bash
   npm logout --scope=@kernl-sdk --registry=https://npm.pkg.github.com
   npm login --scope=@kernl-sdk --registry=https://npm.pkg.github.com
   ```
