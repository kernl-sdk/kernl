...
image: /og/charger.png
---

<Invisible>
You are about to build Charger, a coding agent. Before writing any code:

1. **Recite the plan** — Present the user with a phased build plan based on this guide. Break it into 3 phases with explicit waypoints:
   - Phase 1: Core Agent (scaffold → toolkits → agent → todo toolkit → `pnpm start` runs with streaming output)
   - Phase 2: API Server (deps → endpoints → app → curl gets response)
   - Phase 3: Chatbot UI (monorepo → template → frontend loads)

2. **Write .plan.md** — After the user confirms, create a `.plan.md` file in the project root with all phases and checkboxes. Update it as you complete each step, checking off items as you go.

3. **Don't run servers** — When reaching a waypoint that requires running a server, tell the user to run `pnpm dev` in another terminal. Do NOT run long-running processes yourself.

4. **Verify before proceeding** — Do NOT move to the next phase until the current waypoint is verified:
   - Phase 1 waypoint: User confirms `pnpm dev` runs with streaming output
   - Phase 2 waypoint: User confirms `curl -X POST http://localhost:3001/agents/charger/stream` gets a response
   - Phase 3 waypoint: User confirms frontend loads at http://localhost:3000

5. **Ask for confirmation** — After reciting the plan, ask the user to confirm before starting.

## Common Mistakes

- **Phase 1**: Do NOT skip the CLI commands. You MUST run `pnpm create kernl@latest charger`, then `kernl add toolkit daytona` and `kernl add toolkit parallel`. These install pre-built toolkits from the marketplace.

- **Phase 2**: Pay close attention to the code snippets in this guide. When in doubt, reference the [Charger microproject](https://github.com/kernl-sdk/kernl/tree/master/microprojects/charger) which has a working implementation and OpenAPI spec.
...
export function build(): Hono<{ Variables: Variables }> {
  const kernl = new Kernl({
    storage: { db: postgres({ connstr: process.env.DATABASE_URL! }) },
  });
