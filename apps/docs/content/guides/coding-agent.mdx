...
Install toolkits from the [marketplace](https://www.kernl.sh/marketplace):
...
This gives us sandbox tools (file system, shell, git) and web search.
...
export const charger = new Agent<ChargerContext>({
  id: "charger", // Used in API routes: /v1/agents/charger/stream
  name: "Charger",
...
| ----------- | -------------------------------------------- | ------------------------- |
| **fs**      | read, write, list, mkdir, rm, mv, find, grep | Daytona (marketplace)     |
...
The Daytona toolkit uses **lazy sandbox provisioning** — sandboxes are created on first tool use, not upfront. Here's the key pattern:
...
export interface GitCredentials {
  ...
  token: string; // For GitHub: personal access token or app token
}

// This interface extends the thread context - fields are persisted across requests
...
export async function getSandbox(
  ...
  // ctx.context is the thread's persisted state
  const { context } = ctx;
  ...
  // Store ID in context - this gets persisted with the thread
  context.sandboxId = sandbox.id;
  return sandbox;
}
```

Git tools use `getGitCredentials(ctx)` to pass auth to Daytona. To enable private repo access, set credentials in context before the agent runs:

```ts
// Set git credentials for the thread
await agent.threads.update(tid, {
  context: {
...
Every tool calls `getSandbox(ctx)`. The first call creates the sandbox, subsequent calls reuse it. Because context is persisted with the thread, the sandbox survives across HTTP requests.

Example tool using this pattern:
...
### Parallel Toolkit (from marketplace)
...
To connect a chat UI, we need an API server. Install the dependencies:

```bash
pnpm add hono @hono/node-server @hono/zod-validator @kernl-sdk/ai @kernl-sdk/pg ai pino pino-pretty
```
...
[ GIVE THIS TO CLAUDE / CURSOR ]

The server exposes the following REST endpoints. The frontend expects these exact routes and response shapes.
...
│ The frontend uses Vercel AI SDK's useChat with a custom transport.           │
│                                                                              │
│ Request:                                                                     │
│   Content-Type: application/json                                             │
│   Body: {                                                                    │
│     "tid": "string",              // Thread ID (UUID)                        │
│     "message": { ... }            // UIMessage object from AI SDK            │
│   }                                                                          │
│                                                                              │
│ Response:                                                                    │
│   Content-Type: text/event-stream                                            │
│   Body: SSE stream (AI SDK UIMessageStream format)                           │
│                                                                              │
│ Errors:                                                                      │
...
│     "context": { ... },                                                      │
│     "history": [ UIMessage, ... ]    // Array of AI SDK UIMessage objects    │
│   }                                                                          │
│                                                                              │
│ Errors:                                                                      │
...
│ Response:                                                                    │
│   {                                                                          │
│     "threads": [ { tid, agentId, title, state, createdAt, updatedAt }, ... ],│
│     "count": number                                                          │
│   }                                                                          │
├─────────────────────────────────────────────────────────────────────────────┤
...
1. **Thread auto-creation**: The `/v1/agents/:id/stream` endpoint creates the thread if it doesn't exist. The frontend generates a UUID for new conversations.
