import type { RegistryItem } from "./types";

export const toolkits: RegistryItem[] = [
  {
    name: "github",
    type: "registry:toolkit",
    title: "GitHub",
    description: "GitHub connector via GitHub Copilot MCP",
    icon: "https://registry.kernl.sh/icons/github.svg",
    category: "developer-tools",
    env: ["GITHUB_TOKEN"],
    files: [{ path: "toolkits/github/index.ts" }],
  },
  {
    name: "linear",
    type: "registry:toolkit",
    title: "Linear",
    description: "Linear issue tracking and project management",
    icon: "https://registry.kernl.sh/icons/linear.svg",
    category: "project-management",
    env: ["LINEAR_API_KEY"],
    files: [{ path: "toolkits/linear/index.ts" }],
  },
  {
    name: "exa",
    type: "registry:toolkit",
    title: "Exa",
    description: "Real-time web search and code context via Exa AI",
    icon: "https://registry.kernl.sh/icons/exa.svg",
    category: "search",
    env: ["EXA_API_KEY"],
    files: [{ path: "toolkits/exa/index.ts" }],
  },
  {
    name: "parallel",
    type: "registry:toolkit",
    title: "Parallel",
    description: "Real-time web search and content extraction via Parallel AI",
    icon: "https://registry.kernl.sh/icons/parallel.svg",
    category: "search",
    env: ["PARALLEL_API_KEY"],
    files: [{ path: "toolkits/parallel/index.ts" }],
  },
  {
    name: "alai",
    type: "registry:toolkit",
    title: "Alai",
    description: "Transform text content into fully designed presentations",
    icon: "https://registry.kernl.sh/icons/alai.svg",
    category: "productivity",
    env: ["ALAI_API_KEY"],
    files: [
      { path: "toolkits/alai/index.ts" },
      { path: "toolkits/alai/client.ts" },
    ],
  },
  {
    name: "agentmail",
    type: "registry:toolkit",
    title: "AgentMail",
    description: "Email API for AI agents - send, receive, and manage emails",
    icon: "https://registry.kernl.sh/icons/agentmail.svg",
    category: "communication",
    dependencies: ["agentmail@^0.1.13"],
    env: ["AGENTMAIL_API_KEY"],
    files: [
      { path: "toolkits/agentmail/index.ts" },
      { path: "toolkits/agentmail/client.ts" },
      { path: "toolkits/agentmail/messages/send.ts" },
      { path: "toolkits/agentmail/messages/list.ts" },
      { path: "toolkits/agentmail/messages/get.ts" },
      { path: "toolkits/agentmail/messages/reply.ts" },
      { path: "toolkits/agentmail/drafts/create.ts" },
      { path: "toolkits/agentmail/drafts/get.ts" },
      { path: "toolkits/agentmail/drafts/send.ts" },
      { path: "toolkits/agentmail/drafts/list.ts" },
      { path: "toolkits/agentmail/drafts/update.ts" },
      { path: "toolkits/agentmail/drafts/delete.ts" },
      { path: "toolkits/agentmail/messages/update.ts" },
      { path: "toolkits/agentmail/threads/list.ts" },
      { path: "toolkits/agentmail/threads/get.ts" },
    ],
  },
  {
    name: "bundlesocial",
    type: "registry:toolkit",
    title: "Bundle Social",
    description: "Social media API for scheduling and publishing posts across multiple platforms",
    icon: "https://registry.kernl.sh/icons/bundlesocial.svg",
    category: "communication",
    dependencies: ["bundlesocial@^2.31.0"],
    env: ["BUNDLESOCIAL_API_KEY", "BUNDLESOCIAL_TEAM_ID"],
    files: [
      { path: "toolkits/bundlesocial/index.ts" },
      { path: "toolkits/bundlesocial/client.ts" },
      { path: "toolkits/bundlesocial/constants.ts" },
      { path: "toolkits/bundlesocial/posts/create.ts" },
      { path: "toolkits/bundlesocial/posts/list.ts" },
      { path: "toolkits/bundlesocial/posts/get.ts" },
      { path: "toolkits/bundlesocial/posts/update.ts" },
      { path: "toolkits/bundlesocial/posts/delete.ts" },
      { path: "toolkits/bundlesocial/posts/retry.ts" },
      { path: "toolkits/bundlesocial/analytics/post.ts" },
      { path: "toolkits/bundlesocial/analytics/account.ts" },
      { path: "toolkits/bundlesocial/comments/create.ts" },
      { path: "toolkits/bundlesocial/comments/list.ts" },
      { path: "toolkits/bundlesocial/comments/delete.ts" },
    ],
  },
];
