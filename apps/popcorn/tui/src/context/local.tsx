...
import { Provider } from "@popcorn/shared/provider/provider"
  ...
  init: () => {
    ...
    const agent = iife(() => {
      ...
      const [agentStore, setAgentStore] = createStore<{
        current: string  // stores agent ID
      }>({
        current: agents()[0]?.id ?? "codex",
      })
      ...
      return {
        ...
        current() {
          return agents().find((x) => x.id === agentStore.current) ?? agents()[0]
        },
        set(id: string) {
          if (!agents().some((x) => x.id === id))
            return toast.show({
              variant: "warning",
        ...
        move(direction: 1 | -1) {
          batch(() => {
            let next = agents().findIndex((x) => x.id === agentStore.current) + direction
