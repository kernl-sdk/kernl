...
import { Locale } from "@popcorn/shared/util/locale";
import type {
  Tool,
  ReadTool,
  WriteTool,
  ...
  TaskTool,
  QuestionTool,
} from "@popcorn/shared/tool/tools";
import {
  useKeyboard,
  useRenderer,
...
import { LANGUAGE_EXTENSIONS } from "@popcorn/shared/lsp/language";
import parsers from "@popcorn/shared/parsers";
...
import { Filesystem } from "@popcorn/shared/util/filesystem";
...
function UserMessage(props: {
  ...
  const files = createMemo(() =>
    props.parts.flatMap((x) => (x.type === "file" ? [x] : [])),
  );
  const sync = useSync();
  const { theme } = useTheme();
  const [hover, setHover] = createSignal(false);
  ...
  const compaction = createMemo(() =>
    props.parts.find((x) => x.type === "compaction"),
  );
  ...
  const toolprops = {
    ...
    get permission() {
      ...
      const permissionIndex = permissions.findIndex(
        (x) => x.tool?.callID === props.part.callID,
      );
...
function Bash(props: ToolProps<BashTool>) {
  const { theme } = useTheme();
...
function Write(props: ToolProps<WriteTool>) {
  const ctx = use();
  const { theme, syntax } = useTheme();
  ...
  return (
    <Switch>
      <Match when={props.metadata.diff !== undefined}>
        <BlockTool
          title={"← Write " + normalizePath(props.input.filePath!)}
          part={props.part}
        >
          <box paddingLeft={1}>
            <diff
...
function Glob(props: ToolProps<GlobTool>) {
  return (
    <InlineTool
      icon="✱"
...
function Read(props: ToolProps<ReadTool>) {
  return (
    <InlineTool
      icon="→"
...
function Grep(props: ToolProps<GrepTool>) {
  return (
    <InlineTool
      icon="✱"
...
function List(props: ToolProps<ListTool>) {
...
function Task(props: ToolProps<TaskTool>) {
  const { theme } = useTheme();
  const keybind = useKeybind();
  const { navigate } = useRoute();
  const local = useLocal();
  ...
  const current = createMemo(() =>
    props.metadata.summary?.findLast((x: { state: { status: string } }) => x.state.status !== "pending"),
  );
  const color = createMemo(() =>
    local.agent.color(props.input.subagent_type ?? "unknown"),
  );
        ...
        >
          <box>
            <text style={{ fg: theme.textMuted }}>
              {props.input.description} ({props.metadata.summary?.length}{" "}
              toolcalls)
            </text>
            <Show when={current()}>
              <text
...
function Edit(props: ToolProps<EditTool>) {
  const ctx = use();
  const { theme, syntax } = useTheme();
  ...
  const diagnostics = createMemo(() => {
    ...
    return arr.filter((x: { severity: number }) => x.severity === 1).slice(0, 3);
  });
...
function Patch(props: ToolProps<PatchTool>) {
  const { theme } = useTheme();
  return (
    <Switch>
